...
//   selftest                           -> F1-F5 fixtures, structural (no browser launch)
//   oauth-login --slot N [--headless]  -> v0.1.0 stub (forward-spec); exit 50
...
function cmdOauthLogin(args) {
  const slot = args.flags.slot;
  if (slot === undefined) {
    process.stderr.write('usage: oauth-login --slot <N> [--headless]\n');
    return 4;
  }
  // v0.1.0 forward-spec stub — real flow lands in v0.2.
  process.stderr.write(
    `oauth-login: NOT IMPLEMENTED in v${VERSION} (forward-spec).\n` +
    `slot=${slot} headless=${!!args.flags.headless}\n` +
    `Tracking: need-singularity/browser-harness#oauth-v2\n`
  );
  return 50;
}
...
function usage() {
  process.stderr.write(
    ...
    `  harness selftest                    run F1-F5 structural fixtures\n` +
    `  harness oauth-login --slot N        oauth flow (v0.1.0 = stub)\n` +
    `\n` +
    `env:\n` +
