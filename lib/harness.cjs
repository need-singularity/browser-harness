...
//   selftest                           -> F1-F9 fixtures, structural (no browser launch)
...
// v0.3.1: symmetric remote selftest (tests/selftest_remote.cjs ships +
//         inlines into the SSH payload — F1/F2/F3/F6 plus F5-remote /
//         F7-remote / F8-remote); exit-52 SCP-back skipped when local
//         and remote slot files are byte-identical (sha256).
...
function usage() {
  process.stderr.write(
    ...
    `  harness selftest [--target {mac|ubu1|ubu2|fleet}]                 run F1-F9 structural fixtures\n` +
