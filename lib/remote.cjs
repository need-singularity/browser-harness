...
async function main() {
  ...
  switch (CTX.subcmd) {
    ...
    case 'selftest': {
      // remote selftest — F1/F2/F3/F6 only (F4 is a parseArgs check
      // that's CLI-side; F5 is a probe subprocess invocation that
      // requires bin/browser-harness which isn't shipped to the remote).
      // We re-run F1/F2/F3/F6 as the remote-survivable subset.
      let fails = 0;
      try {
        ...
        if (missing.length) { process.stderr.write('  FAIL F1 missing: ' + missing.join(',') + '\\n'); fails++; }
        else process.stdout.write('  PASS F1 factory exports complete (remote)\\n');
      } catch (e) { process.stderr.write('  FAIL F1 ' + e.message + '\\n'); fails++; }
      try {
        const a = factory.slotStatePath('');
        const c = factory.slotStatePath(9);
        if (a !== null) { process.stderr.write('  FAIL F2 empty slot\\n'); fails++; }
        else if (typeof c !== 'string' || !c.endsWith('slot-9.json')) { process.stderr.write('  FAIL F2 path\\n'); fails++; }
        else process.stdout.write('  PASS F2 slot path = ' + c + ' (remote)\\n');
      } catch (e) { process.stderr.write('  FAIL F2 ' + e.message + '\\n'); fails++; }
      try {
        await factory.dispose(null);
        await factory.dispose(undefined);
        process.stdout.write('  PASS F3 dispose idempotent (remote)\\n');
      } catch (e) { process.stderr.write('  FAIL F3 ' + e.message + '\\n'); fails++; }
      try {
        const oauthMissing = ['runOauthLogin', 'isStateValid', 'safeUrlPrefix']
          .filter(k => typeof oauth[k] !== 'function');
        if (oauthMissing.length) { process.stderr.write('  FAIL F6 oauth exports\\n'); fails++; }
        else {
          const code = await oauth.runOauthLogin({});
          if (code !== 4) { process.stderr.write('  FAIL F6 slot-required exit=' + code + '\\n'); fails++; }
          else process.stdout.write('  PASS F6 oauth exports + --slot required (remote)\\n');
        }
      } catch (e) { process.stderr.write('  FAIL F6 ' + e.message + '\\n'); fails++; }
      if (fails === 0) {
